		...
		if(req.body.id_product && req.body.id_customer) {
			...
			client.query(queryText, [req.body.id_product], function(err, product_result) {
				...
				if(product_result.rowCount > 0) {
					...
					var use_weight = parseInt(req.body.weight);
					var save_query_text;
					var bind_array;

					var queryText = 'SELECT id, quantity FROM cart WHERE id_customer = $1 AND id_product = $2;'
					client.query(queryText, [req.body.id_customer, req.body.id_product], function(err, quantity_result, weight_result) {
						// handle an error from the query
						if(handleError(err)) return;
						...
						} /*else if(weight_result.rowCount > 0) {
							use_weight += parseInt(weight_result.rows[0].weight);
							save_query_text = "UPDATE cart SET weight = $1, date_updated = $2 WHERE id = $3 returning id, weight;"
							bind_array = [use_weight, dateutil.date(), weight_result.rows[0].id];
							//console.log("Found an item already with weight of " + weight_result.rows[0].weight + " setting weight to " + use_weight);
						} */else {
							save_query_text = 'INSERT INTO cart (id, date_created, date_updated, id_product, id_customer, quantity, weight) VALUES ($1, $2, $3, $4, $5, $6, $7) returning id, quantity;'
							bind_array = [uuid.v4(), dateutil.date(), dateutil.date(), req.body.id_product, req.body.id_customer, use_quantity, use_weight];
...
exports.readCart = function(req, res) {
	...
	pool.connect(function(err, client, done) {
    	...
    	// handle an error from the connect
		if(handleError(err)) return;
		var queryText = 'SELECT c.*, (p.unit_price::numeric * c.quantity) AS line_total, (p.unit_weight * c.quantity) AS total_weight FROM cart c LEFT JOIN products p ON (p.id = c.id_product) WHERE c.id_customer = $1;';
...
exports.deleteCartLine = function(req, res) {
	...
	pool.connect(function(err, client, done) {
		...
		// Validate then insert
		if(req.params.id) {
			// queryText is not working properly. Throws error when in proper quotations? Console log response?
